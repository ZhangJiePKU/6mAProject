...
file2=open(kz_file_nearA,"r") #(Note: For WGA samples, this part should be removed.)
...
# step 2: extract the coordinate --> pos:T neg:A
        T_pos=[l for l,m in enumerate(base) if m == "A"]
...
#                       if b == len(base)-1:
                        ...
                        if T_pos[n+1]-T_pos[n] > 1:
                                nearA.append(T_pos[n]+1)
                        else:
                                for i in range(len(T_pos)-1-n):
                                        if T_pos[n+i+1]-T_pos[n+i] > 1:
                                                nearA.append(T_pos[n+i]+1)
                                                break
                                        else:
                                                if n+i+1 == len(T_pos)-1:
#                                                       if T_pos[len(T_pos)-1] == len(base)-1:
                                                                nearA.append("-")
                                                else:
                                                        continue
